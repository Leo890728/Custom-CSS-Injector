<template>
  <div :class="[
    'fixed top-4 right-4 flex items-center justify-between gap-3 min-w-80 p-3 rounded-lg shadow-lg border z-50 animate-slide-in',
    {
    ...
    <div class="flex items-center gap-2">
      ...
      <!-- 資訊圖標 -->
      <svg v-else-if="type === 'info'" class="w-5 h-5 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
    ...
    <button 
      class="text-current opacity-70 hover:opacity-100 transition-opacity p-1 rounded hover:bg-black hover:bg-opacity-10" 
      @click="$emit('close')"
    >
